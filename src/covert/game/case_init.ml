...
let create (srv:Services.t) ?last_crime_choice (w:World.t) =
  ...
  let region, locs, orgs, mm = loop None 0 in
  {
    ...
    cur_org=Org.cia;
    enemy_anxiety=0;
    agent_autoescape=None;
    agent_jailbreak=None;
    d={
...
let make_agent_for_role_ (s:Services.t) role_id chosen roles agents (v:t) =
  ...
  let rec loop n =
    ...
    let org_id, loc_id =
      ...
      o, l
    in
    if Case.hq_kind v org_id loc_id |> Option.is_none then loop (n+1) else
    (* Check mastermind *)
    ...
    let agents = if Role.hardcoded_action_bit1 role then
        Agent.S.remove_known_data_ (Agent.Id.of_int 16) `Known_face agents
      else
        agents
    in
  ...
  let min_dist_locs org_id =
    let _, min_loc, min_loc2 =
      Loc.Map.fold (fun loc_id _ ((min_dist, min_loc, _) as acc) ->
        match Case.hq_kind v org_id loc_id with
        | None -> acc
        | Some _ ->
    ...
    let org_id, loc_id = Utils.do_while
      (fun () ->
        ...
        let loc_id = Utils.do_while
          ...
          (fun loc_id -> Case.hq_kind v org_id loc_id |> Option.is_none)
        in
        org_id, loc_id)
