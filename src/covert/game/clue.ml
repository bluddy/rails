...
(*         (case, clue_ids) *)
(*       in *)
(*       let case, clue_ids = discover_in_role case in *)
(*       let acc = case, clue_ids in *)
...
(*       let ctr = 2 in *)
(*       Action.Map.fold (fun action_id action acc -> *)
(*         match Action.G.send_agent action with *)
(*         | None -> acc *)
(*         | Some a_agent when Agent.Id.(a_agent = agent_id) -> *)
(*             let event_id =  *)
(**)
(*       ) *)
(*       (Case.G.actions case) *)
