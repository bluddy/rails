...
and 'state mode =
  ...
  | CityOffersToBuyStockMode of ('state, bool, int * int * Station.kind * Owner.t) modalmenu
...
let make_generic_screen ?(send_delayed=false) ?(start_fn=fun _ -> ()) render_fn =
  GenericScreen {
    render_fn;
    send_delayed_fn=send_delayed;
    start_fn;
  }
