...
type stock_broker_ui_msg = 
  ...
  | StockBought of {player_idx: Owner.t; stock: Owner.t; cost: int}
  | StockSold of {player_idx: Owner.t; stock: Owner.t; cost: int}
  | BankruptcyDeclared of {player_idx: Owner.t}
  | Takeover of {player_idx:Owner.t; stock: Owner.t}
  | MoneyTransferredFrom of {player_idx: Owner.t; company: Owner.t; amount: int}
  | MoneyTransferredTo of {player_idx: Owner.t; company: Owner.t; amount: int}
...
type t =
  ...
  | PriorityShipmentDelivered of {player_idx: Owner.t; shipment:Priority_shipment.t; bonus:int}
