...
let run_checks trains params =
  ...
  let ui_msgs =
    Trainmap.fold_mapi_in_place ~f:(fun idx acc train ->
      let periodic = Train.get_period params.fiscal_period_year train in
      let acc = if Money.(periodic.revenue = zero) then Ui_msg.TrainNoRevenue(idx)::acc else acc in
      ...
      let maintenance_cost =
        ...
        let no_maint_penalty = if train.has_maintenance then 0 else 2 in
        train.maintenance_cost + added_maint + no_maint_penalty in
      let acc = if maintenance_cost >= Train.get_engine_cost train then Ui_msg.TrainOldEngine(idx)::acc else acc in
      let periodic = Train.update_periodic fiscal_next_year periodic (fun _ -> Train.make_periodic ()) in
