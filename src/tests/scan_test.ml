...
let%expect_test "print map" =
  ...
  in
  print_map map;
  [%expect {| {"map":[[4,{"dirs":[["Left"],["Right"]],"kind":["Track",["Single"]],"ixn":false,"player":["Human",0]}],[5,{"dirs":[["Left"],["Right"]],"kind":["Track",["Single"]],"ixn":false,"player":["Human",0]}],[3,{"dirs":[["Left"],["Right"]],"kind":["Track",["Single"]],"ixn":false,"player":["Human",0]}]],"width":3,"height":3} |}]
...
let%expect_test "train_scan map one train" =
  ...
  let trains = Trainmap.add (dummy_train (8, 10) Right) trains |> fst in
...
let%expect_test "train_scan map two trains" =
  ...
  let trains = Trainmap.empty ()
    ...
    |> Trainmap.add @@ dummy_train (14, 10) Right |> fst
  in
  S.scan_station_block tracks trains (5, 10) Right player |> print_sscan;
  [%expect {|
    2,
...
let%expect_test "train_scan map two trains same place" =
  ...
  let trains = Trainmap.empty ()
    |> Trainmap.add @@ dummy_train (8, 10) Right |> fst
    |> Trainmap.add @@ dummy_train (8, 10) Right |> fst in
  S.scan_station_block tracks trains (5, 10) Right player |> print_sscan;
  [%expect {|
    2,
    `Single |}]
